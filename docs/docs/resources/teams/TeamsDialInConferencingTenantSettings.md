...
| **EnableNameRecording** | Write | Boolean | Specifies whether the name of a user is recorded on entry to the conference. This recording is used during entry and exit notifications. Set to $true to enable name recording, set to $false to bypass name recording. The default is $true. | |
...
| **UseUniqueConferenceIds** | Write | Boolean | Specifies if Private Meetings are enabled for the users in this tenant. PARAMVALUE: $true | $false | |
